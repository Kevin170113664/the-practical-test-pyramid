...
所有常见的应用都会和一些外部环境做集成（数据库，文件系统，和别的应用交互的网络请求）。这些常常是写单元测试中需要忽略的部分，这将带来妥善的隔离和运行迅速的测试。然而，应用和其他系统的交互内容需要施加测试。集成测试这时候就能伸出援手。他们测试的是和你应用集成的所有外部依赖。
...
对于自动化测试来说，你不仅需要运行你自己的应用，也需要运行你与之集成的组件。如果你正在测试和数据库的集成，那你要在跑测试的时候运行数据库。为了测试能否从硬盘里读取文件，你需要先保存一个文件到硬盘上然后在集成测试中去读取它。
...
我提到过，单元测试是一个模糊的术语，对于集成测试而言，更是如此。对于一些人来讲，集成测试意味着测试链接到其他系统的整个应用（不隔离外部服务，数据库）。我则喜欢把集成测试狭义化，每次只测试一个集成点，测试的时候把其他外部依赖，数据库用测试替身来替代。和针对测试替身以及真正实现的契约测试结合在一起，可以看到这样的集成测试更快，更加独立，并且更容易debug。
...
狭义的集成测试存在于服务的边界。从概念上来说，这样的测试总是在触发导致应用和外部依赖(文件系统，数据库，别的服务)集成的行为。数据库集成测试看起来长这样：
...
1. 启动数据库
2. 连接应用到数据库
3. 在你的代码里触发一个往数据库写数据的函数
4. 通过读取数据库，查看期待的数据是不是写到了数据库里
...
另一个例子，测试一个通过REST API和外部服务集成的例子可能是像这样的:
...
1. 启动应用
2. 启动一个外部服务的实例(或者这是一个具有相同接口的测试替身)
3. 触发一个需要读取外部服务API的函数
4. 检查应用是否能正确解析返回结果
...
你的集成测试就像单元测试一样，可以是白盒测试。一些框架可以让你的应用在仿造掉外部依赖的情况下启动，这样你就可以去检查交互是否正确。
...
为序列化或者反序列化数据的代码写集成测试可能会比你想象中要来得频繁。试想以下场景：
...
撰写徘徊于系统边界的集成测试保证了对这些外部系统的读写操作正常工作。
...
当你写狭义的集成测试时你应该在你本地运行外部依赖：启动一个本地的MySQL数据库，针对本地的ext4文件系统进行测试。如果需要和外部服务集成，可以在本地运行该服务的实例，或者构建一个具有仿造行为的服务并在本地运行。
...
如果没办法本地运行一个第三方服务的实例，那么可以考虑运行一个专用实例并在集成测试中指向它。避免在自动化测试里集成真实的生产环境。针对生产环境爆出上千个测试请求是个惹人生气的好办法，因为你会扰乱日志(最好的情况)甚至会对该服务产生Dos攻击(最差的情况)。通过网络和一个服务集成是广义集成测试的一大特征，这也会让你的测试更慢，更难编写。
...
关于测试金字塔，集成测试位于比单元测试更高的层级。集成缓慢依赖(例如文件系统或者数据库)让集成测试运行起来比隔离了这些依赖的单元测试要来的慢。他们可能比小型并且独立的单元测试难写，毕竟你需要让外部依赖在你的测试中运行起来。然而，它的优势在于建立了你对应用能正确访问外部依赖的自信，单元测试并不能帮你建立这种自信。
...
```PersonRepository``` 是代码里唯一的数据库类。它依赖于*Spring Data*，我们并没有实际去实现它。只需要继承```CrudRepository```接口并声明一个方法名。其他的东西全都由Spring给我们实现。
...
对于```CrudRepository```接口，Spring Boot提供了完整的CRUD方法例如```findOne```, ```findAll```, ```save```, ```update```和```delete```。我们自定义的方法（```findByLastName()```）继承自这些基础功能并实现了根据last name获取Persons对象的功能。Spring Data会解析方法返回类型和方法名，从而知道你需要根据数据库里的某个字段进行查找。
...
虽然Spring Data已经实现了和数据库交互的大部分功能，我还是想写一个数据库集成测试。你可能会反对，认为这是在测试框架，我们应该避免测试不属于我们开发的代码。然则，我坚信在这里写一个集成测试是至关重要的。首先它测试了我们自定义的findByLastName方法实际的行为如我们所愿。次之它证明了我们的数据库类用了Spring的装配是正确的，它是能正确连接到数据库的。
...
为了让你能更容易把测试运行起来(不必真的装一个PostgreSQL数据库)，我们的测试会连接到一个内存*H2*数据库。
...
我已经在```build.gradle```里定义H2作为测试依赖。而测试目录下的```application.properties```没有定义任何```spring.datasource```属性。这意味着Spring Data会使用内存数据库，它会在classpath里找到H2来跑我们的测试。
...
当你用int profile运行真正的应用时(例如把```SPRING_PROFILES_ACTIVE=int```设置到环境变量里)，它会连接到```application-int.properties```里定义的PostgreSQL。
...
我知道这涉及到了很多Spring的知识。为了达到目的，你必须仔细研读[大量文档](https://docs.spring.io/spring-boot/docs/current/reference/html/boot-features-sql.html#boot-features-embedded-database-support)。结果代码表面上很容易看懂，但是如果你不知道Spring的一些知识点是很难加以理解的。
...
从头来看的话，测试使用内存数据库其实是有点风险的。毕竟我们集成测试针对的数据库和我们生产用的数据库不一样。你可以自己决定使用Spring提供的简单代码，亦或者是写详细且明确的实现。
...
**消费方驱动的契约测试**(Consumer-Driven Contract tests，CDC测试)是让[消费者驱动契约实现]
(https://martinfowler.com/articles/consumerDrivenContracts.html)。使用CDC，接口消费者会写测试检查接口返回的所有数据是不是他们想要的。消费方团队会发布这些测试从而让提供方可以获取到这些测试并执行。提供方团队现在可以一边运行CDC测试一边开发他们的API了。一旦所有测试通过，他们就知道已经实现了所有消费方想要的东西。
