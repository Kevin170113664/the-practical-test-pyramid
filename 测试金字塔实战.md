...
*图8：每一个接口都有提供方（或者发布者）和消费者（或者订阅者）实体。接口之间的规范可以视为是一个契约。*
...
当你把服务消费方和服务提供方分散到不同的团队去时，你就需要清楚地了解这些服务之间的借口（也就是我们所讲的**契约**）。传统的公司一般是通过以下的方式解决这个问题：
...
**消费方驱动的契约测试*（Consumer-Driven Contract tests，CDC测试）是让[消费者驱动契约实现](https://martinfowler.com/articles/consumerDrivenContracts.html)。使用CDC，接口消费者会写测试，检查接口是不是返回了他们想要的所有数据。消费方团队会发布这些测试从而让提供方可以轻松获取到这些测试并执行。提供方团队现在可以一边运行CDC测试一边开发他们的API了。一旦所有测试通过，他们就知道已经实现了所有消费方想要的东西。

> > 译文里有的地方译「消费方」，有的地方译「消费者」，有何区别呢？
...
这种方式允许提供方团队只实现必要的东西（让设计保持简约，[YAGNI](https://martinfowler.com/bliki/Yagni.html)等）。提供方团队需要持续地获取并运行这些CDC测试（从他们的构建Pipeline里），从而能立即发现任何打破契约的代码变更。如果有代码变更破坏了接口，CDC测试应该会执行失败，这样可以防止破坏性改动上线。当这些测试保持通过，团队就可以做任何他们想做的改动而不需要担心其他团队。CDC这种方式将会让你看到类似这样的过程：
...
[Pact](https://github.com/realestate-com-au/pact)应该是这些日子里最显眼的工具。它有全面的方式来撰写提供方或者消费方的测试，它给你提供开箱即用的打桩隔离其他服务的方式，它也能让你方便的和其他团队交换CDC测试。Pact已经被移植到多平台上，可以和JVM语言一起使用，例如Ruby，.NET，JavaScript等。
...
如果你观察得仔细，你会发现```WeatherClientConsumerTest```真的和```WeatherClientIntegrationTest```很相似。这次我们用协议来取代Wiremock打桩的功能。事实上消费方测试就是集成测试，我们用打桩的方式隔离了第三方服务，定义了我们期待的响应，检查我们客户端可以正确处理响应。某种意义上来说```WeatherClientConsumerTest```是一个狭隘集成测试。基于Wiremock的好处是，它每次运行都会创建一个*协议文件*(你可以在这里找到```target/pacts/&pact-name>.json```)。这个协议文件用特殊的JSON格式描述了这个契约的期待的结果。这也能用于验证我们打桩的服务行为就跟真实服务是一样的。我们可以把这个协议文件交给提供API的团队，他们可以根据这个文件的期待结果来撰写提供方测试。这样的话他们就能测试他们的API是不是满足了所有期待结果。
...
提供方团队拿到协议文件后，需要实现提供方测试来读取协议文件，打桩隔离掉一些测试数据，用他们提供的服务运行检查是否为协议文件中期待的结果。
...
测试用户界面不必非得在端到端的方式下完成。根据技术栈不同，把后端用桩隔离开的话，写UI测试可以如同对前端js代码写单元测试那般简单。
