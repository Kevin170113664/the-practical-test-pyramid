...
我提到过，单元测试是一个模糊的术语，对于集成测试而言，更是如此。对于一些人来讲，集成测试意味着测试链接到其他系统的整个应用（不隔离外部服务，数据库）。我则喜欢把集成测试看得更狭隘，每次只测试一个集成点，测试的时候把其他外部依赖，数据库用测试替身来替代。和针对测试替身以及真正实现的契约测试结合在一起，可以看到这样的集成测试更快，更加独立，并且更容易debug。
...
狭隘的集成测试存在于服务的边界。从概念上来说，这样的测试总是在触发导致应用和外部依赖(文件系统，数据库，别的服务)集成的行为。数据库集成测试看起来长这样：
...
当你写狭隘的集成测试时你应该在你本地运行外部依赖：启动一个本地的MySQL数据库，针对本地的ext4文件系统进行测试。如果需要和外部服务集成，可以在本地运行该服务的实例，或者构建一个具有仿造行为的服务并在本地运行。
