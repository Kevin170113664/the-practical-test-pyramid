...
[偶尔](https://martinfowler.com/bliki/UnitTest.html)有人会把stub掉所有依赖的测试称为独居单元测试，把和依赖有交互的测试成为群居单元测试（Jay Fields的[《Working Effectively with Unit Tests》](https://leanpub.com/wewut)这本书里创造了这些概念）。如果有空你可以继续深究下去，读一读不同思想流派各自的[利弊](https://martinfowler.com/articles/mocksArentStubs.html)在哪。（这段翻译得真好）
...
(at the end of the day 是「到最后」的意思，是个俚语。看美剧看来的，哈哈哈哈。我还记得当时句子是，at the end of the day, we all die)。
...
Mocks和Stubs是两种不一样的测试替身（测试替身还不止这两种）。很多人会混用Mock和Stub这两个概念。我认为，准确的用词会好点，并且最好能将它们各自的特性谙熟于心。你可以使用测试替身来替换掉真实的对象，给它一个可以更方便地测试的实现。（precise 好像译成了 practice；production 我觉得不是生产环境，而只是和测试代码相对的产品代码（实现代码））
...
并不是在单元测试时我们才使用测试替身。还有很多精妙的测试替身能以非常可控的方式来模拟整个系统的功能。然而，在单元测试里使用mocks和stubs的概率会更高（取决于你是喜欢群居风格还是独居风格的开发者），这主要是因为现代语言和库使得构建mocks和stubs变得更加简单了。（第一句翻得好~）
...
你的单元测试跑起来应该非常快。在一般的机器上跑完数千个单元测试应该只需要几分钟。为了得到快速的单元测试，你应该独立地测试代码库的每一小块，并避免进行真实的数据库操作、文件系统操作，或者发送真实的 HTTP 请求（使用 mocks 和 stubs 来隔离这一部分）。（末句的句式 do A and do B to keep C，逻辑关系应该是：为了达到 C 的目的，你应该做 A 和 B。原译逻辑为，做 A 时应保证做 B，以此来达到 C 的目的。我觉得这个关系没对。）
...
1. 我觉得英文中的 your xxx, we xxx，这些可以不必直译成你的 xxx，我们的 xxx。根据中文语感裁剪
2. flow 我一直理解是工作流
3. 之前跟仝老师交流时，他讲过[「TDD 里测试是送的」](https://github.com/linesh-simplicity/linesh-simplicity.github.io/issues/178)。这里拿过来借用，将 producing 译为「送」，其实也不确定是否确定
4. 关于 TDD 能「驱动出设计」这事，是有个前提的，那就是你懂设计，或至少要懂得如何思考设计。如此 TDD 才能驱使你思考。原文没有提到这一层，不知会不会造成读者误会。要是说没有接触过设计模式，没有接触过整洁代码，没有接触过设计原则，说 TDD 能帮忙驱动出设计我肯定是不信的
...
一个单元测试类至少应该测试**这个类的公共接口**。私有方法不能直接测试的原因是你不能从测试类直接调用它们。受保护的和包私有的方法可以被测试类直接调用（如果测试类和生产代码类的包结构是一样的），但是测试这些方法可能就太过了。（翻的好）
