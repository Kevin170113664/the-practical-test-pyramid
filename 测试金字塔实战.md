...
2. flow 我理解是工作流
3. 之前跟仝老师交流时，他讲过[「TDD 里测试是送的」](https://github.com/linesh-simplicity/linesh-simplicity.github.io/issues/178)。这里拿过来借用，将 producing 译为「送」，不知道是否合适
...
reflect 直译好像不太通顺。「不在测试代码中反映出代码的内部结构」，我理解就是测试的写法不要了解代码的实现，于是意译为「不要在单元测试里与实现代码的内部结构相耦合」。
