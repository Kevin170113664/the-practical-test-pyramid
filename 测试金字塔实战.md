...
你测试组合的基石将是单元测试。你的单元测试需要保证你代码库里的某个单元（测试主体）能按照预期那样工作。单元测试在你的测试组合里测试的范围是最窄的。它的数量在测试组合中应该远多于其他类型的测试。
...
如果你去问三个人同样的问题，“单元”在单元测试里意味着什么，你会获得四种带有细微差别的答案。在某种程度上这是一个你自己定义的问题，这个问题没有标准答案也是完全可以的。
...
如果你正在使用函数式语言，一个单元最有可能指的是一个函数。你的单元测试将使用不同的参数调用这个函数，保证它返回期待的结果。在面向对象语言里，一个单元的范围可以从单一方法扩大到整个类。
...
一些人主张在你的测试主体下的所有合作者（比如在测试里被你的类调用的其他类）都应该被mock或者stub掉，这样就能建立完美的隔离，避免副作用和复杂的测试准备。还有些人主张仅仅执行起来慢或者有更大副作用的合作者（比如对数据库读写或者产生网络请求的类）应该被mock或者stub掉。
...
[偶尔](https://martinfowler.com/bliki/UnitTest.html)有人会把stub掉所有依赖的测试成为独居单元测试，把和依赖有交互的测试成为群居单元测试（Jay Fields的[《Working Effectively with Unit Tests》](https://leanpub.com/wewut)这本书里创造了这些概念）。如果你有空你可以继续深究下去，读一读不同思想流派的[利弊](https://martinfowler.com/articles/mocksArentStubs.html)在哪。
...
在今天结束之前，决定采用群居方式的单元测试还是独居的单元测试其实并不重要。写自动化测试才是重点。就我自己而言，我发现我自己一直都是让这两种方式并存。如果使用真正的合作者不方便的话，我就会去mock或者stub掉。如果我觉得引用真正的合作者能让我在测试里更有信心，我会仅仅stub掉service最外层的依赖。
...
Mocks和Stubs是两种不一样的测试替身（测试替身还不止这两种）。很多人会混淆Mock和Stub的概念而交替使用。我认为能了解这两者的实践和特定属性当然是最好的了。你可以使用测试替身来重写掉生产环境对象的实现，让你能更方便地去测试。
...
换句话说，这意味着你会用一个假的实现来代替真的（例如一个类，一个模块或者函数）。假的实现被调用时会和真的实现一样返回同样的东西（响应同一个调用时），但是你必须在单元测试开始前就定义好假实现的预设响应。
...
并不是在单元测试我们才使用测试替身，还有很多精妙的测试替身能用来模拟程序的方方面面功能。然而，在单元测试里你大概率会遇到很多mocks和stubs（这取决于你是一个喜欢群居或者独居单元测试的开发者），这很大程度是因为现代语言和库让我们很轻易地就构建起mocks和stubs。
...
你的单元测试跑起来应该非常快，在一般的机器上你可以期待几分钟内跑完数千个单元测试。测试你代码库的小块功能时，应该隔离开数据库的操作，文件系统的操作和HTTP网络请求的操作，以此保证你的测试能跑得飞快。
...
一旦你掌握了写单元测试的诀窍，你写起来就能越来越顺畅。Stub掉外部依赖，构建一些输入数据，在测试里调用你的测试主体，然后检查返回值是不是你所期待的。看看[测试驱动开发](https://en.wikipedia.org/wiki/Test-driven_development)，让你的单元测试指引你的开发；如果你使用得当，测试驱动开发将帮你进入一个良好循环，在自动生成全面的自动化测试组合时，也会创造出良好的可维护性强的设计。当然，测试驱动开发并不是银弹。但是建议你尝试一下，去看看它是否适合你。
...
单元测试有个好处，你可以为你所有的产品代码类写单元测试，不需要管它们的功能如何或者它们在你代码结构中属于怎样的层次。你可以像测试数据库，领域类或者文件读写那样测试controller。良好的开端，从坚持**一个生产类就有一个测试类**的法则开始。
...
一个单元测试类至少应该测试**这个类的公共接口**。私有方法不能直接测试的原因是你不能从测试类直接调用它们。受保护的和包私有的方法时可以通过测试类直接调用（考虑到测试类和生产代码类的包结构是一样的），但是测试这些方法可能太过了。
...
编写单元测试有一条细则：它们应该保证你代码所有的路径都被测试到（包括正确路径和边缘路径）。同时它们不应该和代码实现紧紧联系在一起。
...
测试如果过于接近生产代码（的实现），很快就会令人讨厌。当你重构你的代码时（快速回顾：重构意味着改变代码的内部结构而不改变其对外的行为）你的单元测试会通过不了。
...
这样的话你就损失了单元测试的一大功能：充当代码变更的保护网。你愿意忍受这种每次重构都会跑不过的，间接提升工作量的测试吗？谁会想去写这种吃力不讨好的测试呢？
...
取而代之的是，我们应该怎么做？不要在你的单元测试里反映你代码内部的结构。测试显式的行为。试想
...
*如果我输入x和y，这个方法会先调用A类，然后调用B类，接着返回A类加上B类的结果？*
...
我经常听人争辩说单元测试（或者说TDD）正在变得无意义，因为必须以提高测试覆盖率为目的去测试所有的方法。他们经常引用这样的场景：过于激昂的团队领导强硬地让他们为getter和setter，抑或是别的种类繁复的代码施加测试，以期望于达到100%测试覆盖率。
...
确实你应该测试公共接口。但是更重要的是，**不要去测试微不足道的代码**。别担心，[Kent Beck说这样是OK的](https://stackoverflow.com/questions/153234/how-deep-are-your-unit-tests/)。你不会因为测试getter，setter抑或是其他简单的实现（打个比方，没有任何
条件逻辑的简单实现）而得到任何东西。把时间挤出来，你就能多参加一个会了，万岁！
...
这种模式也可以应用于别的更高层次的测试。在任何情况下，它们能保证你的测试是易于阅读并保持一致的。除此之外，这种结构往往能用简短的语句写出更具表现力的测试。
...
一个针对hello(lastname)方法的单元测试可以长这样：
...
我们在用[JUnit](http://junit.org/)来写单元测试，一个Java的标准测试框架。我们使用[Mockito](http://site.mockito.org/)来stub掉真正的PersonRepository类。这个stub允许我们在测试里重新定义PersonRepository被调用后产生的响应。stub能让我们的测试更简单，可预测并且更容易组织测试数据。
...
依照Arrange，Act，Assert的结构，我们写了两个单元测试 - 一个是正常的场景，另一个是搜索不到的场景。首先，正常场景创建了一个新的person对象，然后告诉mock的类在被以“Pan”作为参数调用时返回这个person对象。这个测试接着调用了被测试方法。最后它断言返回值是等于期待结果的。
...
第二个测试和第一个类似，但是场景变为被测试方法根据传进来的参数找不到相应的person对象。
