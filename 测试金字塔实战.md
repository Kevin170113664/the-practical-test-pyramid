...
notes：at the end of the day 是「到最后」的意思，是个俚语。看美剧看来的，哈哈哈哈。我还记得当时句子是，at the end of the day, we all die。
...
1. precise 好像译成了 practice
2. production 我觉得不是生产环境，而只是和测试代码相对的产品代码（实现代码）
...
notes：末句的句式 do A and do B to keep C，逻辑关系应该是：为了达到 C 的目的，你应该做 A 和 B。原译逻辑为，做 A 时应保证做 B，以此来达到 C 的目的。我觉得这个关系没对。
...
notes：reflect 直译好像不太通顺。「不在测试代码中反映出代码的内部结构」，我理解就是测试的写法不要了解代码的实现，于是意译为「不要在单元测试里与实现代码的内部结构相耦合」。
