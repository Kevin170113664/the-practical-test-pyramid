...
好的构建流水线能第一时间告诉你什么东西被破坏了。你一定不会想等了一小时后发现，最新的改动因为几个简单的单元测试挂掉了。假如流水线真的需要等很长时间才有反馈，那你很有可能等不及看而回家了。想要在一开始的数秒或者数分钟内知道结果的话，就需要把跑得很快的测试放在流水线里更靠前的阶段。与之相反，把一些跑得慢，通常是覆盖范围广的测试们放在流水线中靠后的阶段，不让他们推迟你获得快速反馈的时机。因此，部署流水线里不同阶段应该放什么测试不应该由测试类型来区分而应该以它们跑的速度以及覆盖面来区分。记住这一点后，把一些覆盖范围很窄并且跑得很快的集成测试放在跟单元测试同一阶段是一个合理的决定，因为它们能给你快速反馈，这种时候就不要对所有不同类型的测试划清界限了。
